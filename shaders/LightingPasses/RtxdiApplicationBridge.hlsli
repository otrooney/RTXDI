...
float RAB_GetLightSampleTargetPdfForSurface(RAB_LightSample lightSample, RAB_Surface surface)
{
    ...
    float3 s;
    if (surface.roughness == 0 || (lightSample.lightType == PolymorphicLightType::kVirtual && g_Const.virtualLightContribution == VirtualLightContribution_DIFFUSE))
        s = 0;
    else
...
{
    PolymorphicLightSample pls = PolymorphicLight::calcSample(lightInfo, uv, surface.worldPos, g_Const.virtualLightClampingRatio);
